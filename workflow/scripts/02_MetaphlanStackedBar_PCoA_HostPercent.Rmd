...
```{r}
flagstat<-read_tsv("../01_data/BMOE006_TreatmentScreen/mlm_out/flagstat_mappedpercent.tsv", col_names = FALSE) %>%
  rename(Sample = "X1") %>%
...
pb <- ggplot(flagstat, aes(x = treatment_method, y = PercentHuman)) +
  ...
  xlab("Treatment") +
  ggtitle("Human Read Abundance (Bowtie2)") + 
...
ggsave("../05_figures/19_Bowtie2HumanReadAbundance.pdf", units="in", width = 6, height=6)
```
